...
2. **Workflow IDs and correlation.** Build IDs with `buildWorkflowId()` — dot-delimited
   ...
   memo are set. See [ADR-0011](docs/adr/0011-workflow-id-and-correlation-tagging.md).
3. **State machines are authored, diagrams are generated.** Domains follow prepare → decide →
   ...
   [ADR-0009](docs/adr/0009-chassaing-decider-split.md)). After changing any `states.ts` /
...
npm test              # 230 tests, ~7s, no containers     npm run typecheck
