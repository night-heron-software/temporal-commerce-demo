...
6. **Workflow IDs & Correlation (ported from nightheron-mono ADR-0011)**: Build workflow IDs with `buildWorkflowId(DEMO_STORE_ID, domain, entityId)` from `src/temporal/contracts/constants.ts` — never inline `{storeId}.{domain}.{entityId}` strings (lint-enforced). At workflow **starts**, spread `buildWorkflowStartOptions()` so the correlation Search Attributes (`CorrelationId`, `StoreId`, `Domain`, `OrderId`, `CartId`) and memo are set. `correlationId` is the journey's **own UUID** — minted at cart creation, owned by the cart workflow, cached in a scoped cookie, and **not** the cartId (ADR-0031, reversing the 2026-08-12 cartId-as-correlation amendment: a 30-day cookie reviving a cart id spliced two journeys onto one correlation key, and correlation-keyed reads have no discriminator). The one-query-per-journey property survives via the cached key. It is REQUIRED — singletons opt out with explicit `undefined`, and a missing journey key THROWS (`requireCorrelationId` in `src/temporal/contracts/constants.ts`) rather than falling back to an entity id. Downstream workflow starts inherit the value from their parent's `CorrelationId` Search Attribute. Inside activities, read the ambient value with `currentCorrelationId()` (`src/lib/correlation-context.ts`) — the worker interceptors thread it; never hand-pass it through activity arguments.
7. **`guard → prepare → decide → evolve` (ADR-0003/0009/0024/0026/0029)**: Domain state machines are authored with `defineMachine()` from `src/temporal/framework` — `defineDomain()`/`transitions()`/`definePureState()` were retired in ADR-0024 and are now lint-banned. There is no `*-decider.ts`; each domain's deciders live in its `states.ts`, authored as one `CommandBlock` per command (ADR-0029): `guard` refuses before any I/O, `prepare` runs activities, `decide` is the pure decision, `routes` declares where its events lead (ADR-0026), and `evolve` folds them into context. `decide` is pure and synchronous — no activities, no clock (`meta.timestamp` / the `at` hook argument carry deterministic time; lint-enforced), no id generation (inject via the command from `prepare`). I/O belongs in `prepare` and in event-keyed `effects`, which are declared at state and machine level, deliberately outside the block. Changes to `states.ts` files **require** co-located `*.test.ts` unit tests.
...
4. **Correlation-keyed audit tables.** The `inventory_history` journal is partitioned by
   `correlation_id` (the journey's own UUID — ADR-0031; `__platform__` for correlation-less ops), and
