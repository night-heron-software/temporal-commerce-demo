...
6. **Workflow IDs & Correlation (ported from nightheron-mono ADR-0011)**: Build workflow IDs with `buildWorkflowId(DEMO_STORE_ID, domain, entityId)` from `src/temporal/contracts/constants.ts` — never inline `{storeId}.{domain}.{entityId}` strings (lint-enforced). At workflow **starts**, spread `buildWorkflowStartOptions()` so the correlation Search Attributes (`CorrelationId`, `StoreId`, `Domain`, `OrderId`, `CartId`) and memo are set. `correlationId` **is the cartId** for a cart journey (ADR-0011's 2026-08-12 amendment; ADR-0022 one-lifecycle-id): one id retrieves everything a cart ever did, across every run and checkout. The per-run UUID mint this replaced gave the same cart a different id on every revival, so no single query returned its history. It is REQUIRED — singletons opt out with explicit `undefined`. Downstream workflow starts inherit the value from their parent's `CorrelationId` Search Attribute. Inside activities, read the ambient value with `currentCorrelationId()` (`src/lib/correlation-context.ts`) — the worker interceptors thread it; never hand-pass it through activity arguments.
...
4. **Correlation-keyed audit tables.** The `inventory_history` journal is partitioned by
   `correlation_id` (the journey id — the cartId; `__platform__` for correlation-less ops), and
