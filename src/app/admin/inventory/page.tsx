...
export default function AdminInventoryPage() {
  ...
  const filteredReservations = reservations.filter((row) => {
    if (
      searchQuery &&
      !row.blankSku.toLowerCase().includes(searchQuery.toLowerCase()) &&
      !row.cartId.toLowerCase().includes(searchQuery.toLowerCase()) &&
      !row.reservationId.toLowerCase().includes(searchQuery.toLowerCase())
    )
      return false;
    return true;
  });
    ...
    <div className="max-w-7xl mx-auto p-8">
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
          ...
          <StatCard
            ...
            accent={stats.totalReserved > 0 ? 'amber' : undefined}
          />
          <StatCard
            label="Available"
          ...
          <StatCard
            ...
            accent={stats.activeReservations > 0 ? 'blue' : undefined}
          />
          <StatCard
            label="Low Stock"
            value={stats.lowStockSkus.toLocaleString()}
            accent={stats.lowStockSkus > 0 ? 'red' : undefined}
          />
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mb-4">
        <input
          type="text"
          placeholder={
            tab === 'stock' ? 'Filter by SKU...' : 'Filter by SKU, cart, or reservation ID...'
          }
          value={searchQuery}
        ...
        {tab === 'stock' && (
          <div className="flex gap-1">
            <FilterPill active={stockFilter === 'all'} onClick={() => setStockFilter('all')}>
              All
            </FilterPill>
            <FilterPill
              active={stockFilter === 'reserved'}
              onClick={() => setStockFilter('reserved')}
            >
              Reserved
            </FilterPill>
            <FilterPill active={stockFilter === 'low'} onClick={() => setStockFilter('low')}>
              Low Stock
            </FilterPill>
          </div>
        )}
      ...
      {isLoading ? (
        ...
        filteredStock.length === 0 ? (
          <div className="text-center py-16 bg-white dark:bg-zinc-800 rounded-lg text-zinc-500">
            {stock.length === 0
              ? 'No inventory stock data. Run seed-inventory first.'
            ...
            <table className="w-full border-collapse bg-white dark:bg-zinc-800 rounded-lg overflow-hidden shadow">
              <thead>
                <tr className="bg-zinc-100 dark:bg-zinc-700 border-b-2 border-zinc-200 dark:border-zinc-600">
                  <th className="px-4 py-3 text-left font-semibold text-zinc-700 dark:text-zinc-300 text-sm">
                    Blank SKU
                  </th>
                  <th className="px-4 py-3 text-left font-semibold text-zinc-700 dark:text-zinc-300 text-sm">
                    Fulfiller
                  </th>
                  <th className="px-4 py-3 text-right font-semibold text-zinc-700 dark:text-zinc-300 text-sm">
                    Total
                  </th>
                  <th className="px-4 py-3 text-right font-semibold text-zinc-700 dark:text-zinc-300 text-sm">
                    Reserved
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-zinc-700 dark:text-zinc-300 text-sm w-32">
                    Utilization
                  </th>
                </tr>
              </thead>
              <tbody>
                {filteredStock.map((row) => {
                  const pct =
                    row.totalStock > 0
                      ? ((row.totalStock - row.availableStock) / row.totalStock) * 100
                      : 0;
                  return (
                    <tr
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right tabular-nums">
                        <span
                          className={
                            row.availableStock < LOW_STOCK_THRESHOLD
            ...
            <tbody>
              {filteredReservations.map((row) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusClasses(row.status)}`}
                    >
                      {row.status}
                    </span>
                  </td>
...
function StatCard({
  ...
  return (
    <div className="p-4 bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700">
      ...
      <div
        className={`text-2xl font-bold tabular-nums ${accent ? accentClasses[accent] : 'text-zinc-900 dark:text-zinc-100'}`}
      >
        {value}
      </div>
    </div>
