...
- **OMS / order** — the _Order Management System_ domain (`src/temporal/oms/`). The workflow ID
  domain segment is `order` (`demo.order.{orderId}`); prose and directory names say "OMS." One
  domain, two spellings, historical reasons.
- **Fulfillment** — the domain that manages _all_ fulfiller orders for one customer order
...
- **Reservation** — a hold on stock for a cart line item, taken via Cassandra compare-and-set
  ...
  placed) → **transfer** (fulfillment start) → **fulfill** (delivery; decrements physical stock)
  — or out early via **release** (timeout / cancellation / removal), **cancel** (post-confirm
  order cancellation), or **expire** (TTL sweep). A released hold can be **resurrected**
  (availability-checked, back to `TEMPORARY`) by the pay-after-expiry confirm path.
...
- **Update vs. signal** — _updates when a caller needs an answer, signals when nobody's waiting._
...
- **Communication** — a persisted record of an email sent to the customer: a row in the
  `customer_communications` Cassandra table (`((order_id), sent_at, seq)`) plus a doc in the
  `communications` ES index, written by the `sendEmail()` choke point
  (`src/lib/email-service.ts`). Surfaced on the order trace, the admin order detail, and the
  shopper's order history; see [specs/order-communications.md](../specs/order-communications.md).
